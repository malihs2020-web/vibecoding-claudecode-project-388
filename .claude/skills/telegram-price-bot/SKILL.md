...
2. Выполнить `python "D:\07 vibecode\parcer\receive.py" --init` — это помечает всю текущую переписку с ботом как уже прочитанную, чтобы не реагировать на сообщения, отправленные до старта.
...
1. Выполнить `python "D:\07 vibecode\parcer\receive.py"` и разобрать JSON-массив из stdout.
...
3. Массив не пустой → обработать каждый объект по порядку (по возрастанию `update_id`):
   ...
   h. Отправить текст ответа, **не подставляя его как буквальный текст прямо в командную строку** — текст ответа может содержать символы, ломающие shell-кавычки (кавычки `"` внутри размеров товара, `$` в суммах, обратные кавычки, переносы строк). Вместо этого: сначала записать текст ответа в отдельный временный файл через инструмент Write (без участия shell), затем вызвать `python "D:\07 vibecode\parcer\send.py" "$(cat "<путь-к-временному-файлу>")"` (путь к временному файлу тоже в кавычках — сам путь к проекту содержит пробел) — подстановка команд `$(cat file)` читает содержимое файла как есть и подставляет его одним аргументом, не переинтерпретируя `"`, `$` и обратные кавычки, которые уже были внутри текста.
