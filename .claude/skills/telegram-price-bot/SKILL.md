...
2. Выполнить `python "D:\03 Вайкодинг\sreda\проекты\parcer\receive.py" --init` — это помечает всю текущую переписку с ботом как уже прочитанную, чтобы не реагировать на сообщения, отправленные до старта.
3. Запустить регулярную проверку через skill `loop` **в динамическом режиме, без фиксированного интервала** (то есть без ведущего токена вида `3m` во входе `loop`) — промпт должен указывать выполнить раздел «Проверка (что делать на каждой проверке)» этого файла (`.claude/skills/telegram-price-bot/SKILL.md`). Фиксированный интервал (`/loop 3m ...`) здесь **не использовать**: он создаёт повторяющийся cron через `CronCreate`, а команда «хватит слушать» (`ScheduleWakeup` с `stop: true`) такой cron не отменяет — цикл продолжил бы работать в фоне. Динамический режим (без ведущего интервала) самопланируется через `ScheduleWakeup`, и тот же вызов `stop: true` его корректно останавливает — поэтому старт и стоп должны использовать один и тот же механизм.
...
1. Выполнить `python "D:\03 Вайкодинг\sreda\проекты\parcer\receive.py"` и разобрать JSON-массив из stdout.
2. Массив пустой → ничего не писать в чат и не в Telegram; перейти сразу к шагу 5 (запланировать следующую проверку) и дождаться следующего срабатывания цикла.
3. Массив не пустой → обработать каждый объект по порядку (по возрастанию `update_id`):
   ...
   h. Отправить текст ответа, **не подставляя его как буквальный текст прямо в командную строку** — текст ответа может содержать символы, ломающие shell-кавычки (кавычки `"` внутри размеров товара, `$` в суммах, обратные кавычки, переносы строк). Вместо этого: сначала записать текст ответа в отдельный временный файл через инструмент Write (без участия shell), затем вызвать `python "D:\03 Вайкодинг\sreda\проекты\parcer\send.py" "$(cat "<путь-к-временному-файлу>")"` (путь к временному файлу тоже в кавычках — сам путь к проекту содержит пробел) — подстановка команд `$(cat file)` читает содержимое файла как есть и подставляет его одним аргументом, не переинтерпретируя `"`, `$` и обратные кавычки, которые уже были внутри текста.
...
5. В конце каждой проверки — независимо от того, были найдены сообщения или массив был пустым — вызвать `ScheduleWakeup` с `delaySeconds: 180`, тем же `prompt` (исходный ввод `/loop`, с префиксом `/loop `) и коротким `reason`, чтобы запланировать следующую проверку. Если `ScheduleWakeup` сообщает, что запланировать не удалось (например, отключён режим динамического `/loop` в этой среде или цикл достиг максимального времени жизни) — не молчать: явно сообщить пользователю в чат, что прослушивание остановлено, и указать причину.

## Обработка ошибок

- `receive.py` вернул ошибку (ненулевой код возврата / текст ошибки в stderr — например, сеть недоступна) → одна отдельная неудачная проверка не считается сбоем всего цикла: пропустить эту проверку молча, всё равно запланировать следующую проверку через `ScheduleWakeup` (~3 минуты), цикл продолжает работать. Не писать пользователю об единичной заминке. Но если проверки подряд не удаются 3 раза (3 срабатывания цикла подряд с ошибкой) — молчать дальше нельзя: явно сообщить пользователю в чат, что опрос Telegram не работает, и указать последнюю известную причину ошибки.
